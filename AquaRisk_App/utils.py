...
# --- 1. INITIALISATION MEMOIRE (LA FONCTION QUI MANQUAIT) ---
def init_session():
    defaults = {
...
# --- 2. GESTION BASE DE DONNEES (NOUVEAU V60) ---
def init_db():
...
def save_audit_to_db(data):
    """Sauvegarde l'audit actuel"""
    init_db()
    try:
        conn = sqlite3.connect('aquarisk.db')
        c = conn.cursor()
        # On nettoie les objets complexes avant de sauvegarder en JSON
        ...
        conn.commit()
        conn.close()
        return "✅ Audit sauvegardé en base de données !"
    except Exception as e: return f"❌ Erreur sauvegarde : {e}"
...
def calculate_360_risks(data, params):
    ...
    if not data['reut_invest']:
        ...
        risks['Conformité'] = (vol_jour * 1500.0) * (params['pression_legale'] / 100.0)
    else: risks['Conformité'] = 0.0
...
def generate_pdf_report(data):
    ...
    else: pdf.cell(0, 7, "Aucune simulation effectuée.", 1, ln=1)
